<?php
	...
	/**
	 * Library to execute PHP 5 code under a PHP 4 instance
	 *
	...
	class Bridge {
		 ...
		 */
		function Bridge ($script = null, $variables = null, $calls = null) {
		...
		function initialize () {
			...
			if ($_SERVER["argv"][1] == "--php-ghetto-rpc") {
				...
				$this->id = getmypid() . "_" . time() . "_" . uniqid("");
            ...
            # TODO: Allow dependency injection of Medium.
            #       >> a setter for id will be needed.
			$this->medium = new Medium($this->id);
			
			if (!session_id() == "" && !headers_sent())
				session_start();
			
			register_shutdown_function(array(&$this, PHPGR_IS_BACKEND ? "export" : "__destruct"));			
		...
		function export () {
            ...
            # TODO: Flexible configuration to select what will be exported
            #       ... Something like cURL::curl_setopt_array
			$data = array(
		...
		function error ($errno, $errstr, $errfile, $errline) {
			...
			$this->errors[] = array("level"   => $errlevel,
									"message" => $errstr,
									"file"	=> $errfile,
									"line"	=> $errline
							  );
